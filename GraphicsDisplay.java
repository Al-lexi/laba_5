// package bsu.rfe.java.group8.lab5.Leonenko.varA11;
...
public class GraphicsDisplay extends JPanel {
    ...
    public class MouseHandler extends MouseAdapter {
        ...
        public void mouseClicked(MouseEvent ev) {
            if (ev.getButton() == 3) { // Если кликнули правой кнопкой
                if (GraphicsDisplay.this.undoHistory.size() > 0) {
                    // Откат к предыдущему масштабу из истории
                    ...
                    // Если истории нет, сброс масштаба к исходному
                ...
                GraphicsDisplay.this.repaint(); // Перерисовка компонента
            }
        }

        public void mousePressed(MouseEvent ev) {
            if (ev.getButton() == 1) { // Если нажали левую кнопку
                // Проверяем, была ли нажата точка графика
                GraphicsDisplay.this.selectedMarker = GraphicsDisplay.this.findSelectedPoint(ev.getX(), ev.getY());
                // Сохраняем координаты начала перетаскивания
                ...
                if (GraphicsDisplay.this.selectedMarker >= 0) {
                    // Если нажата точка, включаем режим перемещения точки
                    GraphicsDisplay.this.changeMode = true;
                    GraphicsDisplay.this.setCursor(Cursor.getPredefinedCursor(8)); // Cursor.MOVE_CURSOR
                } else {
                    // Если нажата не точка, включаем режим масштабирования
                    GraphicsDisplay.this.scaleMode = true;
                    GraphicsDisplay.this.setCursor(Cursor.getPredefinedCursor(5));  // Cursor.CROSSHAIR_CURSOR
        ...
        public void mouseReleased(MouseEvent ev) {
            if (ev.getButton() == 1) { // Если отпустили левую кнопку
                GraphicsDisplay.this.setCursor(Cursor.getPredefinedCursor(0)); //  Cursor.DEFAULT_CURSOR;
                if (GraphicsDisplay.this.changeMode) {
                    // Если был режим перемещения, отключаем его
                    GraphicsDisplay.this.changeMode = false;
                } else {
                    // Если был режим масштабирования, отключаем его
                    GraphicsDisplay.this.scaleMode = false;
                    // Получаем конечные координаты масштабирования
                    double[] finalPoint = GraphicsDisplay.this.translatePointToXY(ev.getX(), ev.getY());
                    // Добавляем текущее окно просмотра в историю
                    GraphicsDisplay.this.undoHistory.add(GraphicsDisplay.this.viewport);
                    // Создаем новое окно просмотра на основе начальной и конечной точки выделения
    ...
    public class MouseMotionHandler implements MouseMotionListener {
        ...
        public void mouseMoved(MouseEvent ev) {
            ...
            GraphicsDisplay.this.selectedMarker = GraphicsDisplay.this.findSelectedPoint(ev.getX(), ev.getY());
            if (GraphicsDisplay.this.selectedMarker >= 0) {
                // Если курсор над точкой, меняем курсор на MOVE_CURSOR
                GraphicsDisplay.this.setCursor(Cursor.getPredefinedCursor(8)); //  Cursor.MOVE_CURSOR
            } else {
                // Если курсор не над точкой, возвращаем курсор по умолчанию
                GraphicsDisplay.this.setCursor(Cursor.getPredefinedCursor(0)); // Cursor.DEFAULT_CURSOR
            }
            GraphicsDisplay.this.repaint(); // Перерисовка компонента
        }
